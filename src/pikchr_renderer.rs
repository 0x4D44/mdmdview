...
arrow right 200% "Output" above"#;
    ...
    fn test_debounce_for_texture_key_builds_entry() {
        let debounce =
            PikchrRenderer::debounce_for_texture_key("pikchr:0000000000003039:w640:s100:dm0")
    ...
    fn test_debounce_for_texture_key_invalid_returns_none() {
        assert!(PikchrRenderer::debounce_for_texture_key("pikchr:abc:s100:dm0").is_none());
    }
    ...
    fn test_rasterize_svg_uses_base_scale_when_width_bucket_is_zero_and_clamps() {
        ...
        let output = PikchrRenderer::rasterize_svg(&fontdb, svg, 0, 100).expect("rasterize");
    ...
    fn test_rasterize_svg_invalid_svg_returns_error() {
        let fontdb = make_fontdb();
        let err = match PikchrRenderer::rasterize_svg(&fontdb, "<svg", 224, 100) {
    ...
    fn test_process_job_returns_raster_error_for_invalid_cached_svg() {
        ...
        let result = PikchrRenderer::process_job(
            &fontdb,
            RasterJob {
                code_hash,
    ...
    fn test_is_stale_result_invalid_texture_key_is_assumed_fresh() {
        ...
        assert!(
            !renderer.is_stale_result("pikchr:malformed", code_hash),
