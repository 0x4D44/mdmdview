...
## Update
- Added image decode test-only forced raster dimensions and a test to hit the post-decode limit branch.
- Tightened multiline table hover test by asserting a multi-row galley before invoking paint.
- Added Mermaid dump helper coverage for the code-None path.
- Ran targeted tests for the new/updated cases; all passed.
- Branch coverage run timed out twice at shorter timeouts; reran with a longer timeout and completed.
- Latest branch coverage: 90.39% (2257 branches, 217 missed).
- Updated wrk_docs coverage outputs and refreshed the coverage report + improvement plan status.

## Update
- Added test-only forced SVG dimensions and new tests for zero-height and tall SVG scaling branches in image decoding.
- Ran targeted tests for new image decode cases (pass).
- First full branch coverage run timed out at 604s; reran with extended timeout and completed.
- Latest branch coverage: 90.48% (2259 branches, 215 missed).
- Refreshed coverage outputs and updated the report + improvement plan status.

## Update
- Added Markdown renderer tests for marker-only list markers and empty paragraph/blockquote parsing paths.
- Added Mermaid renderer tests for format_dim NaN formatting, fallback measure with shorter lines, and empty dump labels.
- Ran targeted tests for the new cases; all passed.
- Ran full branch coverage (606 lib + 18 main tests); branch coverage now 90.70% (2259 branches, 210 missed).
- Refreshed coverage outputs and updated the report + improvement plan status.

## Update
- Added test helpers for paragraph/image/text/emoji span extraction and updated Markdown renderer tests to exercise both match paths.
- Expanded header parsing and inline break tests to include non-matching cases.
- Ran targeted markdown tests; all passed.
- Ran full branch coverage (606 lib + 18 main tests); branch coverage now 91.20% (2249 branches, 198 missed).
- Refreshed coverage outputs and updated the report + improvement plan status.

## Update
- Expanded mermaid dump-helper coverage (dash/underscore labels + empty-label error) to hit missing branches.
- Ran `cargo test --lib mermaid_renderer::tests::test_mermaid_dump_helpers_sanitize_labels` (pass).
...
- Latest branch coverage: 91.33% (2249 branches, 195 missed).
- Updated coverage outputs, coverage-missing list, report, and improvement plan.

## Update
- Starting next coverage improvement pass; focusing on mermaid_renderer + markdown_renderer missing branches.
- Inspecting coverage-missing-branches.txt and mapping missing lines to test targets.

## Update
- Added mermaid_renderer tests for missing state-end circle parts and unmatched ER rects.
- Added markdown_renderer test for nested list-marker header with any-indent detection.
- Ran targeted tests:
  - `cargo test --lib fix_state_end_circles_missing_parts_noop`
  - `cargo test --lib fix_er_attribute_fills_skips_unmatched_rects`
  - `cargo test --lib escape_table_pipes_parent_indent_list_marker_any_indent`

## Update
- Added Mermaid tests for missing state-end circle parts and for skipping unmatched ER rects.
- Added Markdown test for any-indent nested list markers in table detection.
- Ran branch coverage commands:
  - `cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json`
  - `cargo +nightly llvm-cov --branch --summary-only | Tee-Object -FilePath wrk_docs\coverage-summary.txt`
  - `cargo +nightly llvm-cov --branch | Tee-Object -FilePath wrk_docs\coverage.txt`
- Latest branch coverage: 91.51% (2249 branches, 191 missed).
- Updated coverage outputs, report, and improvement plan.

## Update
- Added Mermaid tests for width fallback when screen width equals available width, oversized bbox translate-y-only, tall SVG clamp, and tiny bbox handling.
- Ensured patch_mermaid_js debug false branch restores when env var pre-set.
- Ran targeted tests:
  - `cargo test --lib rasterize_svg`
  - `cargo test --lib render_block_width_fallback_equal_screen`
  - `cargo test --lib patch_mermaid_js_debug_false_branch`

## Update
- Added Markdown renderer tests for empty/indented fenced code blocks, image-only inline spans, and empty table rows.
- Ran targeted tests:
  - cargo test parse_code_block_
  - cargo test parse_inline_spans_image_only_skips_text_buffer
  - cargo test parse_table_empty_row_skips_row
- Ran branch coverage with json output (long run; several tests logged >60s warnings).
...
- Latest branch coverage: 91.74% (2251 branches, 186 missed).
- Markdown renderer branch coverage now 92.59% (71 missed); mermaid renderer 88.26% (52 missed).

## Update
- Added Markdown renderer tests for unclosed image-only inline spans, empty list paragraphs, image rendering with empty titles, and code blocks without start tags.
- Ran targeted tests:
  - cargo test parse_inline_spans_unclosed_image_only
  - cargo test parse_list_empty_paragraph_skips_block
  - cargo test parse_code_block_without_start_tag
  - cargo test render_inline_span_image_empty_title
- Ran branch coverage:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Latest branch coverage: 91.91% (2251 branches, 182 missed).
- Markdown renderer branch coverage now 93.01% (67 missed).

## Update
- Added Markdown renderer test for deep list-marker table-like lines under parent lists to probe escape-table branches.
- Ran targeted test:
  - cargo test escape_table_pipes_parent_indent_deep_list_marker_table_like_lines
- Ran branch coverage:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Branch coverage remains 91.91% (2251 branches, 182 missed).

## Update
- Added Markdown renderer tests for mismatched inline end tags and table-head noise handling.
- Ran targeted tests:
  - cargo test parse_inline_spans_with_breaks_mismatched_end_tag
  - cargo test parse_table_head_skips_noise
- Ran branch coverage:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Latest branch coverage: 91.96% (2251 branches, 181 missed).
- Markdown renderer branch coverage now 93.11% (66 missed).

## Update
- Simplified Mermaid error formatting to ignore empty message/stack strings and removed redundant state-start replacement guard.
- Added Mermaid tests for remove_svg_attr spacing edge cases and viewBox infinity inputs; added restore_env_var helper for patch debug restore paths.
- Tried a rasterize_svg zero-dimension case; usvg rejected width/height 0, then 0.1 produced (1,1), so the test was removed.
- Ran targeted tests:
  - cargo test test_format_js_error_variants --lib
  - cargo test test_remove_svg_attr_handles_no_space_prefix --lib
  - cargo test test_parse_viewbox_dims_and_format_dim --lib
- Ran coverage commands:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Regenerated wrk_docs/coverage-missing-branches.txt from coverage.json.
- Latest branch coverage: 92.24% (2241 branches, 174 missed); 628 lib tests + 18 main tests; 1 mismatched function warning.

## Update
- Added Mermaid tests for empty error message/stack fallthrough in format_js_error and for even-only ER attribute fills.
- Ran targeted tests:
  - cargo test test_format_js_error_variants --lib
  - cargo test test_fix_er_attribute_fills_handles_even_only --lib

## Update
- Ran full branch coverage and refreshed outputs after adding new Mermaid tests.
- Ran coverage commands:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Regenerated wrk_docs/coverage-missing-branches.txt from coverage.json.
- Latest branch coverage: 92.32% (2241 branches, 172 missed); 629 lib tests + 18 main tests; 1 mismatched function warning.

## Update
- Added Markdown renderer tests for small delta width persistence, short-text hover skip, single-line overflow hover, and duplicate frame width-change handling.
- Ran targeted tests:
  - cargo test test_persist_resizable_widths_skips_small_delta --lib
  - cargo test test_paint_table_text_job --lib
  - cargo test test_handle_width_change_skips_duplicate_frame --lib

## Update
- Ran full branch coverage after Markdown renderer test additions.
- Ran coverage commands:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Regenerated wrk_docs/coverage-missing-branches.txt from coverage.json.
- Latest branch coverage: 92.41% (2241 branches, 170 missed); 633 lib tests + 18 main tests; 1 mismatched function warning.

## Update
- Added Markdown renderer tests for mermaid pending detection and for link hit-testing/click activation using two-frame pointer positioning.
- Adjusted the anchor click test to compute a reliable pointer position before sending the click.
- Ran targeted tests:
  - cargo test test_paint_table_text_job_triggers_anchor_on_click --lib
  - cargo test test_link_at_pointer_inside_text_rect_returns_link --lib
  - cargo test test_has_pending_renders_with_mermaid_pending --lib

## Update
- Ran full branch coverage after link hit-testing and mermaid pending tests.
- Ran coverage commands:
  - cargo +nightly llvm-cov --branch --json --output-path wrk_docs\coverage.json -q
  - cargo +nightly llvm-cov --branch --summary-only --no-run | Tee-Object -FilePath wrk_docs\coverage-summary.txt
  - cargo +nightly llvm-cov --branch --no-run | Tee-Object -FilePath wrk_docs\coverage.txt
- Regenerated wrk_docs/coverage-missing-branches.txt from coverage.json.
- Latest branch coverage: 92.46% (2241 branches, 169 missed); 636 lib tests + 18 main tests; 1 mismatched function warning.

## Update
- Added Markdown renderer test to cover hover-only link handling in table text jobs without triggering anchors.
- Ran targeted test:
  - cargo test test_paint_table_text_job_hover_link_does_not_trigger --lib
