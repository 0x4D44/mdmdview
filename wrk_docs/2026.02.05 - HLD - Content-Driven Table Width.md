...
| V3 | Fixed row height estimation: content-fit mode must pass natural widths to row height estimator, not viewport-based widths. Added §5.9. Updated §5.3 Change 4. Documented Fixed-column resizability change. Updated risk table. |
...
**Change 4**: Row height estimation must use correct widths

The `render_table` closure contains row height estimation logic that calls
`estimate_table_row_widths()`. This function distributes remaining space
to Remainder columns based on `available_for_columns`. In content-fit mode,
`available_for_columns` is computed from the viewport width (too large),
causing Remainder columns to receive hugely overestimated widths. This
makes row heights too short (wider columns = less text wrapping),
causing visible text clipping on the first frame.

**The problem** (inside the `render_table` closure):
```rust
// These variables are captured from the outer scope:
// - available_for_columns: computed from viewport_width (1100px)
// - adjusted_widths: computed against viewport_width
// - scaled_down: false (content easily fits viewport)
//
// In content-fit mode, estimate_table_row_widths distributes
// 1100px of space, but the actual columns only total ~335px.
// Remainder columns get ~900px estimated width → row heights too low.
...
**The fix**: In content-fit mode, bypass `estimate_table_row_widths` and
use `natural_widths` directly for row height estimation:
...
The `content_fits` bool and `natural_widths` Vec must be captured by the
`render_table` closure. Since `content_fits` is `Copy` and
`natural_widths` is already computed in the outer scope, this is
straightforward — the closure already captures many outer variables.
...
### 5.9 Impact on Row Height Estimation

Row heights are estimated on the first frame (when `needs_estimate` is
true) and cached for subsequent frames. The estimation uses approximate
column widths to lay out text and measure wrapping.

**Why this matters**: Row height depends on column width — narrower columns
cause more text wrapping, which requires taller rows. If the height
estimate uses the wrong column widths, rows will be clipped (too short)
or have excessive whitespace (too tall) on the first frame.

**Current code path** (inside `render_table` closure):
1. `estimate_table_row_widths()` computes approximate column widths
2. For each row, `estimate_table_row_height()` measures text layout
3. Results cached in `table_metrics` for next frame

In **viewport-fill mode**, the existing logic is correct — it uses
`available_for_columns` based on viewport width, which matches the
actual allocation.

In **content-fit mode**, the actual column widths will be
`natural_widths`, NOT the viewport-based estimates. The fix (§5.3
Change 4) passes `natural_widths` directly, bypassing
`estimate_table_row_widths`.

**Self-healing**: Even if the first-frame estimate is slightly off,
the actual rendered row heights are cached and used from the second
frame onward. The fix ensures the first frame is also correct.

### 5.10 Behavioral Note: Fixed Columns in Content-Fit Mode

In content-fit mode, ALL columns (including Fixed-policy columns) use
`Column::initial(width).resizable(true)`. This is a behavioral change
from viewport-fill mode where Fixed columns use `Column::exact(width)`
(non-resizable).

**Consequence**: Users can drag-resize Fixed columns in content-fit
mode. However, `persist_resizable_widths` only saves widths for
Resizable-policy columns. So if a user resizes a Fixed column in
content-fit mode, the change is NOT persisted and will revert on the
next frame.

**Rationale**: This is acceptable for the initial implementation:
- Content-fit tables are typically small; Fixed column widths are
  reasonable for the content
- Making Fixed columns truly exact (`Column::exact`) in content-fit
  mode would prevent egui from adjusting them if the container is
  slightly mis-sized due to rounding
- A future enhancement could persist Fixed column resizes if needed

**Alternative considered**: Use `Column::exact(width)` for Fixed
columns in content-fit mode. Rejected because `Column::exact` is
rigid and doesn't allow egui to make minor adjustments for rounding.
The `Column::initial` approach is more forgiving.
...
**Policy classification** (at 14px):
- Col 0 "Operation": index 0, max_graphemes=16 (≤18), longest_word=9 (≤12) → Fixed 98px
- Col 1 "Time (ms)": matches "time" → Fixed 119px
- Col 2 "Memory (MB)": classified Resizable, but fallback promotes to Remainder
  (remainder_assigned=0, Col 2 is highest-scoring non-Fixed non-first column)
- Col 3 "CPU (%)": Resizable preferred 133px
- Col 4 "Status": matches "status" → Fixed 98px

**Policy-resolved widths**:
- Col 0: Fixed 98px, Col 1: Fixed 119px, Col 2: Remainder min_floor 84px,
  Col 3: Resizable preferred 133px, Col 4: Fixed 98px

**Natural widths** = max(content, resolved):
- Col 0: max(164, 98) = 164px
- Col 1: max(103, 119) = 119px
- Col 2: max(121, 84) = 121px (content wins over min_floor)
- Col 3: max(86, 133) = 133px (policy wins)
- Col 4: max(77, 98) = 98px

**natural_total** = 164 + 119 + 121 + 133 + 98 + 24 = 659px

**effective_width** = min(659, 1100) = 659px

**content_fits** = true

**Result**: Content-fit mode. Table = 659px. Compact, well-proportioned.
...
**Modified function**: `render_table_tablebuilder` (~40 lines changed)
...
- Add `content_fits` branch in row height estimation (use natural_widths)
- Rename `available_width` → `viewport_width` for clarity

Estimated diff: ~80 lines added/changed.
...
| Row height wrong on first frame if natural_widths slightly off | Low | Self-corrects on second frame via cached metrics; natural_widths are close enough |
| Fixed columns resizable in content-fit mode (§5.10) | Low | Acceptable UX; resize reverts on next frame; can enhance persistence later |
