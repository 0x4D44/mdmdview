...
4) **Mermaid blocks trigger outbound HTTP by default**  
   - On construction, four worker threads are spawned and any Mermaid block is POSTed to `https://kroki.io` unless the user opts out (`src/markdown_renderer.rs:673-711`, `src/markdown_renderer.rs:3033-3074`). This violates the repo guidance of “No secrets or network calls” and can leak private documents. Default should be offline (render source or require explicit `--allow-kroki` flag/env), and network should be opt-in.
...
4. Make online Mermaid rendering opt-in; gate worker creation and Kroki POSTs behind a config flag/environment variable with a clear status indicator in the UI.  
