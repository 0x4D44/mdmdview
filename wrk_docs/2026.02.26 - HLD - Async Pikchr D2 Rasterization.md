# HLD: Async Pikchr & D2 Rasterization
...
| `ctx.load_texture()` | 0.1–1ms | **No** — `egui::Context` is `!Send` | **No** — main thread only |
...
───────────                           ─────────────
render_block()                        loop {
  ├─ cache checks                       job = rx.recv()
  ├─ debounce gate                      svg = render_source(code)
  ├─ enqueue job ──── tx.send() ────►   rgba = rasterize(svg)
  ├─ show stale/placeholder             tx.send(result) ─────►
  └─ return                           }
...
pub(crate) struct PikchrRenderer {  // (and D2Renderer)
    ...
    /// Set of texture_keys currently in-flight (enqueued but not yet completed).
...
fn worker_loop(
    ...
    while let Ok(job) = job_rx.recv() {
        ...
        // If main thread dropped result_rx (shutdown), send fails — exit gracefully
...
fn rasterize_svg(
    ...
    let svg = Self::inject_svg_font_family(svg);  // Pikchr only
...
New method, called inside `render_block` (same pattern as Mermaid's `poll_mermaid_results`):
...
fn poll_results(&self, ctx: &egui::Context) -> bool {
    ...
    while let Ok(result) = self.result_rx.try_recv() {
        ...
        self.pending.borrow_mut().remove(&result.texture_key);

        // Always cache SVG (content-addressed, width-independent)
        if let Some(ref svg) = result.svg {
            let dark_mode = result.texture_key.ends_with(":dm1");
            let sk = svg_cache_key(result.code_hash, dark_mode);
...
pub(crate) fn render_block(
    ...
    // 3a. Poll for completed results (may populate the texture we need)
    self.poll_results(ui.ctx());

    // 3b. Re-check texture cache after polling
    if let Some(entry) = self.textures.borrow_mut().get(&texture_key) {
        Self::render_texture(ui, &entry, available_width);
        return true;
    }
    ...
    // 5. Check if already pending for this diagram
    if self.pending.borrow().contains(&texture_key) {
        // Job already in flight — show stale texture or placeholder
    ...
    match self.job_tx.as_ref().unwrap().try_send(job) {
        Ok(()) => {
            self.pending.borrow_mut().insert(texture_key);
        }
        Err(crossbeam_channel::TrySendError::Full(_)) => {
            // Queue full — will retry next frame
        }
        Err(crossbeam_channel::TrySendError::Disconnected(_)) => {
            // Worker died — fall back to synchronous? Or show error?
            // For robustness, show an error message.
        }
    }
...
fn show_stale_or_placeholder(
    ...
    true  // false would mean "not rendered" — but we always render something
}
```
...
impl Drop for PikchrRenderer {  // (and D2Renderer)
    fn drop(&mut self) {
        // Close job channel — worker will see Disconnected and exit
        self.job_tx.take();
...
pub(crate) fn begin_frame(&self) {
    // No per-frame state to reset — rasterized_this_frame is removed.
    // Debounce state is persistent across frames (by design).
}
```

`begin_frame()` becomes a no-op. We retain the method for API consistency with `MermaidRenderer`, and in case future per-frame state is needed.
...
pub(crate) fn release_gpu_textures(&self) {
    ...
    // pending is NOT cleared — in-flight jobs will complete and results
    // will be uploaded on next poll_results() call.
}
```

### Stale Result Detection

Same pattern as Mermaid:
...
| `render_texture()` | **Stays** | Called from both cache hit path and poll_results |
| `render_error_block()` | **Stays** | Called from both cache hit path and poll_results |
| `has_pending()` | **Changes** | Returns `!pending.is_empty()` instead of `false` |
| `begin_frame()` | **Simplifies** | Becomes a no-op (retained for API consistency) |
| `release_gpu_textures()` | **Expands** | Also clears `wanted_bucket`; keeps `pending` intact |
...
5. **Result arrives**: `poll_results()` checks staleness, uploads RGBA to GPU (~0.1ms), updates caches.
6. **Display**: `render_block()` finds the texture in cache on the next frame and displays it.

If the user resizes again before the worker finishes (step 4), `wanted_bucket` will have changed, so the result is detected as stale in step 5 — SVG is cached (cheap) but the expensive GPU upload is skipped.
...
On first render, there is no stale texture to display during the worker processing time. The placeholder shows "Rendering..." text at a fixed height of 60px. Once the result arrives (~50–200ms), the real texture replaces it. This is a one-time cost per diagram.
...
If the worker panics (shouldn't happen — all error paths return `Result`), `try_send` returns `TrySendError::Disconnected`. We show an error message in the diagram slot. This is a non-recoverable error.

### Theme change

Theme change clears all caches (SVG, texture, error) and debounce state. The next render enqueues fresh jobs for all visible diagrams. The worker processes them with the new dark_mode flag.

### Window minimize / restore

`release_gpu_textures()` clears textures and debounce state. On restore, render_block() encounters cache misses and enqueues fresh jobs. The SVG cache is retained, so the worker can skip the source→SVG step and go straight to rasterization.

### Zoom change

Scale bucket changes, producing a new texture_key. Cache miss triggers a new job. SVG cache hit means the worker only needs to rasterize (not re-render source→SVG). The old-scale texture displays until the new one arrives.
...
| File | Changes |
|------|---------|
| `src/pikchr_renderer.rs` | Worker thread, channels, `RasterJob`/`RasterResult` types, `poll_results`, split `rasterize_and_upload`, remove `rasterized_this_frame`, update `render_block`/`has_pending`/`release_gpu_textures`, add `Drop`, add `show_stale_or_placeholder`, add `is_stale_result`/`parse_width_from_texture_key`, update tests |
| `src/d2_renderer.rs` | Same changes as pikchr_renderer.rs (parallel implementation) |
| `src/markdown_renderer.rs` | No changes expected — `begin_frame()`, `has_pending()`, `release_gpu_textures()`, and `render_block()` signatures are unchanged |
...
All existing unit tests for both renderers must continue to pass. Tests that directly call `rasterize_and_upload()` will need to be updated to call `rasterize_svg()` instead (the CPU-only portion).

### New tests

1. **`test_worker_processes_job`** — Submit a job, verify result arrives with RGBA data
...
4. **`test_stale_result_skips_gpu_upload`** — Submit job, change wanted_bucket before polling, verify texture cache is NOT populated
5. **`test_stale_result_caches_svg`** — Same as above but verify SVG IS cached
...
10. **`test_release_gpu_clears_wanted_bucket`** — Verify wanted_bucket cleared on release
