...
| 6.0.0   | 2026-02-14 | Cross-reference review: 1 critical, 1 medium, 2 minor (below)  |

### V6 Review Findings

**Cross-reference of V5 design against actual codebase source files.**

- C6: **CRITICAL: Empty fontdb — text labels won't render.** The design used
  `usvg::Options::default()` which creates an empty `fontdb::Database` with NO
  system fonts loaded. Every other SVG rasterization site in the codebase
  explicitly loads system fonts (`mermaid_renderer.rs:1255-1256`,
  `image_decode.rs:133-136`). Without fonts, usvg silently skips text rendering
  — all Pikchr labels (box text, arrow annotations) would be invisible. Fixed
  by adding `fontdb: Arc<usvg::fontdb::Database>` field to `PikchrRenderer`,
  loading system fonts once in `new()`, and passing it to `usvg::Options` in
  `rasterize_and_upload()`. Sections 5.1.1, 5.1.4, 5.1.6, 5.1.9 updated.
- M9: **MEDIUM: `bucket_width` test assertion will fail.** `assert_eq!(
  bucket_width(640.0), bucket_width(660.0))` fails because `660/32=20.625`
  rounds to 21 (bucket 672), not 20 (bucket 640). Fixed test to use values
  within the same ±16px half-bucket. Section 6.1 updated.
- m11: **MINOR: No import block for `pikchr_renderer.rs`.** Code samples
  referenced `ThemeColors`, `RefCell`, `DefaultHasher`, etc. without showing
  `use` statements. Added explicit import block to section 5.1 preamble.
- m12: **MINOR: Redundant `.map_err(|e| e.to_string())`.** `Pikchr::render()`
  already returns `Err(String)`, so the `.map_err` was a needless clone.
  Removed in section 5.1.5.
...
MarkdownRenderer
    ...
    +-- pikchr: PikchrRenderer         (new, ~300-500 lines)
            |
            +-- fontdb: Arc<fontdb::Database>           (system fonts, loaded once)
...
(Plus the local `LruCache<K, V>` definition copied from `mermaid_renderer.rs`.)
...
pub(crate) fn render_block(
    ...
    match Self::rasterize_and_upload(ui.ctx(), &self.fontdb, &texture_key, &svg, width_bucket, scale_bucket) {
        Ok(entry) => {
            Self::render_texture(ui, &entry, available_width);
...
fn rasterize_and_upload(
    ...
    // Without system fonts, usvg silently skips text rendering —
    // Pikchr labels would be invisible. See mermaid_renderer.rs:1255
    // and image_decode.rs:133 for the same pattern.
...
impl PikchrRenderer {
    pub(crate) fn new() -> Self {
        // Load system fonts once at startup. Without this, usvg's default
        // empty fontdb silently drops all text labels in rasterized SVGs.
        // Takes ~50-200ms on a typical system (one-time cost).
        // Same pattern as mermaid_renderer.rs:1255-1257 and image_decode.rs:133-136.
...
| `fontdb::Database`         | ~5-15 MB (system font metadata) |
...
**Font resolution**: `PikchrRenderer` loads system fonts into its own
`Arc<fontdb::Database>` at construction time (see section 5.1.9). This is the
same pattern used by `mermaid_renderer.rs` and `image_decode.rs`. The fontdb
is not shared between Pikchr and Mermaid — each holds its own `Arc`. This
means system fonts are loaded twice (once per renderer), adding ~50-200ms
of total startup time. If this becomes a concern, a shared `Arc<fontdb::Database>`
could be passed down from `MarkdownRenderer` to both renderers. Deferred:
the duplication is harmless and avoids coupling the two renderers.

Note: Pikchr SVGs use simple text labels (sans-serif, no custom fonts), so
font resolution should work reliably on all platforms with system fonts
available.
