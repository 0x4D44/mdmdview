...
- Move Mermaid block detection and thin wrapper into the new module; keep current behavior (QuickJS or Kroki).
...
- Implement renderer selection env var (embedded | kroki | off) and feature gates.
Deliverables:
...
- Docs updated to describe built-in renderer, settings, and Kroki fallback.
Exit criteria:
- Embedded renderer is the default when feature enabled; Kroki remains optional.
