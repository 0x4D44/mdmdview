...
**Disconnected components**: When a container (or the root graph) has multiple disconnected sub-graphs, each component is laid out independently, then arranged side-by-side (horizontal stacking for top-down layouts, vertical stacking for left-right layouts) with a configurable gap between components.
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="{x} {y} {w} {h}">
  ...
  <!-- Arrowheads as explicit polygons (not SVG markers — better resvg compat) -->
