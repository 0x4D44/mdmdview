# HLD: Mermaid Resize Debounce (V3)

**Date**: 2026-02-25
**Status**: Draft
**Scope**: `mermaid_renderer.rs` (primary), `markdown_renderer.rs` (minor)
**Previous**: V1 — initial design, V2 — fixed 6 issues
**V3 changes**: Fixed 3 issues found during V2 review (see [Review Notes V2 → V3](#review-notes-v2--v3) at end)
...
```
0. NEW: Update wanted bucket (for staleness detection in mechanism B):
   mermaid_wanted_bucket.insert(svg_key, width_bucket)
   (Insert after texture_key computation at line 753, before poll at line 755)
...
4. NEW: Debounce gate
   ...
   c. If entry exists and width_bucket == last_enqueued_bucket:
      → bucket matches last enqueue — texture was evicted from LRU cache
        (if it were still cached, step 2 would have returned early)
      → clear bucket_changed_at (timer no longer needed)
      → proceed to enqueue to rebuild the evicted texture (step 5)
...
- The debounce entry is created/updated only on successful enqueue (step 5), not when first encountered (step 4b). This prevents a QueueFull on first render from creating a debounce entry that would incorrectly suppress future enqueue attempts.
- Step 4c (bucket matches `last_enqueued_bucket`) only triggers when the texture has been evicted from the LRU cache — a rare event that warrants a re-enqueue to rebuild it.
...
When processing a `MermaidResult`, check if the `width_bucket` encoded in the `texture_key` still matches what the UI currently needs. If it doesn't, cache the SVG and any errors (cheap), but skip the RGBA upload (expensive).
...
In `render_block`, after computing `texture_key` (line 753) and before `poll_mermaid_results` (line 755):
```rust
let texture_key = Self::texture_key(svg_key, width_bucket, scale_bucket, bg);
self.mermaid_wanted_bucket.borrow_mut().insert(svg_key, width_bucket);  // ← HERE
...
**V3 change**: Error caching is now done **before** the staleness check. This prevents infinite re-render loops: if a diagram's QuickJS render fails, the error is cached even for stale results, so subsequent frames hit the error cache (step 3) instead of re-enqueueing.

**Note on texture_error for stale results**: When a stale result has `has_svg=true` and `error=Some(...)`, the error is keyed by `texture_key` (width-specific). This means the error is cached for the stale width, not the wanted width. This is harmless: the next render at the correct width will have a different `texture_key`, won't find this error, and will enqueue a new rasterization attempt. If the error is truly width-independent (e.g., SVG parsing failure), it's caught by `mermaid_errors` (svg_key-keyed).
...
### Stale result with error
A stale result that includes an error (QuickJS failure or rasterization failure) caches the error before the staleness check discards the GPU upload. This prevents re-enqueueing the same broken diagram repeatedly. See `poll_mermaid_results` implementation.
...
6. **`test_stale_result_caches_error`** — Deliver stale result with error. Verify error is cached despite staleness (prevents infinite re-render loops).

7. **`test_first_render_no_debounce_delay`** — First render of a new diagram proceeds immediately without debounce.

8. **`test_first_render_queue_full_no_debounce_entry`** — First render with QueueFull: verify no WidthDebounce entry created. Next attempt should retry enqueue, not enter debounce.

9. **`test_theme_change_clears_debounce_state`** — Call `clear_mermaid_cache()`, verify debounce maps are empty.

10. **`test_parse_width_from_texture_key`** — Unit test for key parsing with various formats.

11. **`test_latest_texture_index_updated_on_store`** — Verify `mermaid_latest_texture` maps svg_key to most recent texture_key.

12. **`test_debounce_trailing_edge_timer_reset`** — Width changes A→B, then B→C before timer expires. Verify timer resets on B→C transition (trailing edge). Verify debounce fires 150ms after C, not 150ms after B.

13. **`test_release_gpu_textures_clears_debounce_state`** — Call `release_gpu_textures()`, verify debounce maps cleared.
...
1. **`poll_mermaid_results` call pattern misdescribed**: V1 said it "runs at the start of render_block, before the per-diagram render loop". Actually runs inside `render_block`, once per visible diagram. **Fixed**: Corrected frame flow diagram.

2. **Trailing-edge debounce needed `last_seen_bucket`**: V1's `WidthDebounce` only had `last_enqueued_bucket` + `bucket_changed_at`. **Fixed**: Added `last_seen_bucket` field.

3. **QueueFull on first render created premature debounce entry**: **Fixed**: Entry created only on successful enqueue (step 5).

4. **Debounce map unbounded growth**: **Fixed**: Added cleanup in `release_gpu_textures`. Documented negligible memory impact.

5. **LRU eviction invalidates `mermaid_latest_texture`**: **Fixed**: Documented as accepted limitation.

6. **Stale texture doesn't scale up**: **Fixed**: Documented as accepted aesthetic limitation.

## Review Notes (V2 → V3)

Issues found during V2 review and their resolutions:

1. **Error caching skipped for stale results**: V2's `poll_mermaid_results` did `continue` on stale results, skipping the error handling code. If a stale job failed (e.g., QuickJS crash, rasterization error), the error wasn't cached, causing infinite re-render loops on subsequent frames. **Fixed**: Moved error caching before the staleness check. Errors are always cached regardless of staleness.

2. **`mermaid_wanted_bucket` insertion point ambiguous**: V2 said "after computing width_bucket and before poll_mermaid_results" but `texture_key` computation happens between those two points. **Fixed**: Specified exact insertion point — after `texture_key` computation (line 753), before `poll_mermaid_results` (line 755). Added explicit code context.

3. **Step 4c "proceed to enqueue" lacked rationale**: V2 didn't explain why step 4c (bucket matches `last_enqueued_bucket`) proceeds to enqueue when the texture should be cached. **Fixed**: Added clarifying note that this case only triggers when the texture was evicted from the LRU cache — if it were still cached, step 2 would have returned early. Re-enqueueing to rebuild the evicted texture is correct.
