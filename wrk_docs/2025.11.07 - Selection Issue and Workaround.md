...
1. **TextEdit Widget**: Using read-only TextEdit widgets for paragraphs
   - ✅ Preserves selection on right-click
   - ❌ Loses all formatting (bold, italic, links, etc.)
   - Verdict: Not acceptable as it degrades the viewing experience

2. **SelectableLabel**: Using egui's SelectableLabel widget
   - ❌ Designed for list selection, not text selection
   - ❌ Doesn't solve the right-click issue
   - Verdict: Wrong tool for the job

3. **Selection Tracking**: Attempting to capture and store selected text
   - ❌ Selection is cleared before we can capture it
   - ❌ No egui API to get selected text before it's cleared
   - Verdict: Not possible with current egui APIs

4. **Event Interception**: Trying to handle right-click before selection clears
   - ❌ egui processes the right-click first
   - ❌ No way to prevent the default behavior
...
- Added helpful tip: "💡 Select text, then use Ctrl+C to copy"
