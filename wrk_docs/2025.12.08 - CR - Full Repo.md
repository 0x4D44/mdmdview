...
2) **Cache growth is unbounded within a session** (emoji textures, image textures, kroki SVG cache, table stats cache). While typical docs are small, a long session with many unique emojis/images/diagrams could increase memory. Mitigation: add simple LRU or size cap with eviction; expose a “Clear caches” action in UI.
...
3) **Mermaid/Kroki error surfacing**: failures are cached and skip requeue until content changes; UI only shows the fallback banner. Optional: surface last error tooltip so users know why diagrams didn’t render.
...
- **Kroki worker pool**: bounded queue and connect/read/write timeouts guard against hangs; worker threads are named for debuggability.
...
2) Introduce simple cache limits (emoji/image/kroki/table stats) or a “Clear caches” command to prevent long-session bloat. (P3)
3) Improve feedback for Kroki failures (tooltip or status text). (P4)
