...
## Summary
Add first-class, offline Mermaid rendering by embedding Mermaid JS and executing it in-process to produce SVG, then rasterize to textures for egui. Make the local renderer the default while keeping Kroki as an optional, explicitly enabled fallback. Ensure rendering is non-blocking, cached, and safe by default.

## Problem statement
- The current implementation can render Mermaid using a network service (Kroki). This is not ideal for privacy, offline usage, or reliability.
...
- Otherwise it uses Kroki over HTTP (gated by MDMDVIEW_ENABLE_KROKI).
- Kroki jobs run in a worker pool; results are cached in memory.
...
- For Kroki (if enabled), continue to wrap code with an init directive for theme parity.
...
- mermaid-kroki (optional): keep Kroki support for debugging or fallback.
- If we introduce mermaid-embedded, make it an alias of mermaid-quickjs to avoid breaking existing builds.

### Environment variables
- MDMDVIEW_MERMAID_RENDERER: embedded | kroki | off (default: embedded).
- MDMDVIEW_ENABLE_KROKI: retained only if mermaid-kroki feature is enabled.
...
## Rollout plan
1. Introduce MermaidRenderService and keep existing kroki path behind a feature flag.
2. Enable mermaid-embedded by default and set renderer default to embedded.
3. Deprecate MDMDVIEW_ENABLE_KROKI in docs if the feature is disabled by default.
