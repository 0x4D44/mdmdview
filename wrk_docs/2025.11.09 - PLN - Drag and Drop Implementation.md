...
1. **Test: Drag hover visual feedback**
   ...
   - "📄 Drop files to open" text centered
...
1. **Test: Drop 3 files**
   ...
   - Status bar shows "📋 2 files in queue"
   - Alt+→ opens file2.md
   - Status bar shows "📋 1 files in queue"
   - Alt+→ opens file3.md
...
2. **Test: Drop 10 files**
   ...
   Expected:
   - First file opens
   - Status bar shows "📋 9 files in queue"
   - Can navigate through all with Alt+→
   ```
...
5. **Test: Queue and history interaction**
   ```
   Action:
   1. Open file1.md via File→Open
   2. Drop file2.md, file3.md, file4.md
   3. Press Alt+→ (should open file2.md from queue)
   4. Press Alt+← (should go back to file1.md from history)
   5. Press Alt+→ (should go to file2.md from history, NOT queue)
...
1. **Test: Drop folder with 5 markdown files**
   ...
   - Status bar shows "📋 4 files in queue"
   - Alt+→ cycles through beta, charlie, delta, echo
   ```
...
impl MarkdownViewerApp {
    fn handle_file_drop(&mut self, paths: Vec<PathBuf>) {
        ...
        if !errors.is_empty() {
            ...
            eprintln!(
                "✓ Loaded {} files. {} queued. Use Alt+→ to navigate.",
                valid_files.len() + 1,
                self.pending_files.len()
            );
        }
...
- **Drag and drop**: Drag `.md` files or folders directly into the window
  - Multiple files are queued and accessible via `Alt+→`
...
1. **Full workflow test:**
   ...
   6. Navigate through queued files with Alt+→
   7. Navigate back through history with Alt+←
   8. Close app
   9. Restart app
...
3. **Queue management test:**
   ...
   3. Open different file via File→Open
   4. Verify queue cleared
   5. Alt+← should go to history, not queue
   ```
