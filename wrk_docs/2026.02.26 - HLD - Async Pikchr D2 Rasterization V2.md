# HLD: Async Pikchr & D2 Rasterization (V2)
...
pub(crate) fn release_gpu_textures(&self) {
    ...
    // will be uploaded on next poll_results() call. The wanted_bucket is
    // cleared, so stale detection will discard results for widths that no
    // longer match (see is_stale_result: wanted=None → stale).
}
```
...
10. **`test_release_gpu_clears_wanted_bucket`** — Verify wanted_bucket and debounce cleared on release
