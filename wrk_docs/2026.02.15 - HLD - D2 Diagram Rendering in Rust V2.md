...
**Note**: The `near` keyword (`x.near: y`) is **deferred to v1.1** — it requires constraint-aware layout positioning that `rust-sugiyama` does not support, and implementing constraint-based placement is a significant effort orthogonal to the core pipeline.
...
**Processed in v1**: `label`, `shape`, `style`, `near` (parse only, no layout effect), `width`, `height`, `direction`, `source-arrowhead`, `target-arrowhead`
...
└── crates/
    └── d2/                         # new crate: mdmdview-d2
        ├── Cargo.toml              # [dependencies] petgraph, rust-sugiyama, thiserror
        └── src/
...
4. **Resolve named colors**: Convert `Color::Named` strings to `Color::Hex` via lookup table
5. **Validate**: Check shape types, style values, connection endpoints
6. **Set defaults**: Rectangle shape, default theme colors, etc.
...
| Other | 20px | 12px | Default to rectangle |

These values are initial estimates; they'll be tuned during differential testing against the Go implementation.
...
**This is the hardest phase.** The design follows four sub-phases:
...
D2 handles containers by recursively laying out each container's children as a sub-graph. The algorithm:
...
- **During layout**: Cross-container edges contribute to the rank assignment and ordering at the *lowest common ancestor* container's level. Each endpoint is represented by its containing container at that level.
...
**Fallback option**: If `rust-sugiyama` proves insufficient (e.g., API limitations for our use case), we implement our own Sugiyama on petgraph, using D2's Go dagre wrapper as the specification. The algorithm phases are well-documented:
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="{x} {y} {w} {h}">
  <!-- Definitions (markers, patterns) — must precede referencing elements -->
  <defs>
    <marker id="arrow-{id}" viewBox="0 0 10 10"
...
No JS runtime, no WASM, no C libraries. The D2 crate is pure Rust. Total new dependency footprint is minimal — `rust-sugiyama` + `petgraph` + `thiserror` are small, well-maintained crates.
...
| `rust-sugiyama` API doesn't match our assumptions | Medium | Medium | Verify API against docs/source before building; have fallback plan (own Sugiyama) |
