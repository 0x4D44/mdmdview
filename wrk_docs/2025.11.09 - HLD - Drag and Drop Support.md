...
This document outlines the design for adding drag-and-drop functionality to mdmdview, allowing users to open markdown files by dragging them from their file manager directly into the application window. This feature will improve user experience by providing a more intuitive way to open files, complementing the existing File→Open dialog and command-line argument methods.
...
- **Acceptance Criteria**:
  ...
  - Invalid files show error message (same as File→Open)
...
- **Acceptance Criteria**:
  ...
  - User can navigate forward through queue using Alt+→
...
│ MarkdownViewerApp                               │
├─────────────────────────────────────────────────┤
│ + pending_files: VecDeque<PathBuf>             │  NEW
│ + drag_hover: bool                              │  NEW
│ + fn handle_file_drop(&mut self, files: Vec)   │  NEW
│ + fn process_pending_file(&mut self)           │  NEW
│ + fn render_drag_overlay(&self, ui)            │  NEW
└─────────────────────────────────────────────────┘
         ↓ uses
┌─────────────────────────────────────────────────┐
│ Existing: load_file(path: PathBuf)             │
│ Existing: push_history()                        │
│ Existing: navigate_forward()                    │
└─────────────────────────────────────────────────┘
```
...
1. ✅ Enable drag-drop in viewport configuration
2. ✅ Add `drag_hover` field to `MarkdownViewerApp`
3. ✅ Implement `handle_drag_drop_events()` for single file
4. ✅ Implement `is_valid_markdown_file()` validator
...
1. ✅ Implement `render_drag_overlay()`
2. ✅ Add dashed border and semi-transparent background
...
1. ✅ Add `pending_files: VecDeque<PathBuf>` field
2. ✅ Modify `handle_file_drop()` to queue files
3. ✅ Extend `navigate_forward()` to check pending queue first
4. ✅ Add status bar indicator for queue size
5. ✅ Test with 5, 10, 50 files
...
1. ✅ Implement `scan_directory()` method
2. ✅ Add directory detection in `handle_file_drop()`
3. ✅ Implement file limit (50 files max)
4. ✅ Test with nested directories (should not recurse)
...
2. ✅ Handle non-existent files gracefully
...
### User-Facing Metrics
- **Adoption**: % of users who use drag-drop vs File→Open dialog (if telemetry added)
