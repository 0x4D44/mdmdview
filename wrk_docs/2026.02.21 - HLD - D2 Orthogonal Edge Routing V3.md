# HLD — D2 Orthogonal Edge Routing (V3)

**Date:** 2026-02-21
**Author:** Claude Opus 4.6 + Arthur
**Status:** Draft
**Crate:** `mdmdview-d2` (`crates/d2/`)
**Revision:** V3 — fixes channel grouping bug, clarifies multi-pass structure, tightens edge-case specs
...
|---|-------------|-----------|
| 1 | §3.3 grouped edges by `{src_anc, dst_anc}` pair identity. Edges from the same source to different destinations at the same rank (e.g., `gw→auth` and `gw→orders`) have different pair keys but share the same physical gap — their channels overlap at the gap midpoint. | **Fixed:** Group by physical gap interval `(gap_start, gap_end)` rounded to the nearest pixel. All edges passing through the same gap coordinate their channel positions. Added destination cross-position as sort tiebreaker. |
| 2 | §3.4 described Cloud/Person/Document/Parallelogram as getting "shape boundary" ports via `clip_to_shape()`, but the actual `clip_to_shape` function falls through to `clip_to_rect` for all of these shapes. | **Fixed:** Prose corrected. `compute_port` now has two categories: shapes with dedicated clipping (Diamond, Hexagon, Circle, Oval) and all others (bounding-box placement). |
| 3 | §3.7 same-rank jog only showed direction=right and direction=down, leaving left/up as "analogous". | **Fixed:** Added explicit formulas for all four directions. |
| 4 | §4.3 listed router steps sequentially but didn't make the multi-pass structure explicit. An implementer might attempt a single-pass per-edge loop. | **Fixed:** Phase 3 now describes three explicit passes: classify, allocate, build. |
| 5 | §3.4 didn't address same-side port collision when a node has both outgoing forward edges and incoming same-rank jog edges on the same side. | **Fixed:** Added §6.7 documenting this as a known V1 limitation with workaround guidance. |
| 6 | §4.3 tests only covered direction=right. | **Fixed:** Added tests for all four directions, deep nesting, architecture integration test, and bidirectional edges. |
...
**Example trace for overlapping gaps** (the V2 bug):

Within Services (direction=right), `gw → auth` and `gw → orders` where auth and orders are both at rank 1:

```
gw→auth:   gap_start = gw.right(),  gap_end = auth.x    = rank_offsets[1]
gw→orders: gap_start = gw.right(),  gap_end = orders.x  = rank_offsets[1]
Same physical gap! Both edges grouped together.
Channel allocation spreads them: channel_1 and channel_2 at gap_midpoint ± CHANNEL_SPACING/2.
```
...
1. **Group edges by physical gap interval.** Two edges share a gap if they have the same LCA AND their `(gap_start, gap_end)` values match (rounded to the nearest pixel). This correctly groups edges regardless of whether they share `src_anc`/`dst_anc` pair identity — what matters is that their channel segments occupy the same physical gap.

   **Why pair identity fails:** Consider `gw → auth` and `gw → orders` within a container (direction=right). Auth and orders are at the same rank, so they share the same X position (`rank_offsets[1]`). Both edges have `gap_start = gw.right()` and `gap_end = rank_offsets[1]` — the same physical gap. But their `{src_anc, dst_anc}` pairs differ (`{gw, auth}` vs `{gw, orders}`), so pair-based grouping would allocate them independently to the same channel position, producing overlapping channel segments.

   ```rust
   // Gap key: round to nearest pixel to handle floating-point noise
   let gap_key = (gap_start.round() as i64, gap_end.round() as i64);
   gap_groups.entry((lca, gap_key)).or_default().push(edge_info);
   ```
...
3. **Sort edges within each gap.** Primary sort key: source node's cross-position (center Y for direction=right, ascending). Secondary sort key (tiebreaker for edges from the same source): destination node's cross-position (ascending). This ensures channel segments are ordered consistently, minimizing visual crossings.

   ```rust
   edges_in_gap.sort_by(|a, b| {
       let a_src_cross = src_cross_pos(a);
       let b_src_cross = src_cross_pos(b);
       a_src_cross.partial_cmp(&b_src_cross)
           .unwrap_or(std::cmp::Ordering::Equal)
           .then_with(|| {
               let a_dst_cross = dst_cross_pos(a);
               let b_dst_cross = dst_cross_pos(b);
               a_dst_cross.partial_cmp(&b_dst_cross)
                   .unwrap_or(std::cmp::Ordering::Equal)
           })
   });
   ```
...
**Shapes with dedicated clipping** (Diamond, Hexagon, Circle, Oval): Use `clip_to_shape()` from the node center toward a target point on the desired exit side. The clipped point lands on the actual shape boundary, which may be slightly off the desired cross-axis position — producing a small diagonal at the shape boundary, which is acceptable.

**All other shapes** (Rectangle, Square, Cylinder, Code, Cloud, Person, Package, Queue, Page, Step, StoredData, Document, Parallelogram, Callout, Text): Place ports directly on the bounding box edge at the desired cross-axis position. Note: shapes like Cloud and Person have visual boundaries inside their bounding box, so the port may appear slightly outside the visible shape. This matches the existing `clip_to_shape` behavior, which also falls through to `clip_to_rect` for these shapes.
...
fn compute_port(
    ...
    match shape {
        ShapeType::Diamond | ShapeType::Hexagon
...
When a node has N outgoing edges from the same exit side, sort them by destination cross-position before assigning port indices. This ensures the topmost edge exits at the topmost port, minimizing crossings.
...
Same logic applies to destination entry ports: sort incoming edges by source cross-position, distribute entry ports evenly along the entry edge. Incoming and outgoing ports on the same side are allocated independently (see §6.7 for the known limitation this creates).
...
/// Round to 2 decimal places (reuse existing `c()` in svg_render.rs:24).
...
**Formulas for all four directions:**

```rust
let (jog_primary, src_port_side, dst_port_side) = match direction {
    Direction::Right => (
        src_rect.right().max(dst_rect.right()) + SAME_RANK_JOG,
        Side::Right, // src exits right
        Side::Right, // dst enters from right (jog goes past)
    ),
    Direction::Left => (
        src_rect.x.min(dst_rect.x) - SAME_RANK_JOG,
        Side::Left,  // src exits left
        Side::Left,  // dst enters from left
    ),
    Direction::Down => (
        src_rect.bottom().max(dst_rect.bottom()) + SAME_RANK_JOG,
        Side::Bottom, // src exits bottom
        Side::Bottom, // dst enters from bottom
    ),
    Direction::Up => (
        src_rect.y.min(dst_rect.y) - SAME_RANK_JOG,
        Side::Top,   // src exits top
        Side::Top,   // dst enters from top
    ),
};
```

**Route construction for direction=right:**
...
**Note:** The destination port is on the SAME side as the exit (not the opposite as in normal forward routing), because the jog goes past both nodes. The enter segment goes backward from the jog into the destination shape.

**Route construction for direction=down:**

```rust
let route = vec![
    src_port,                                           // shape bottom edge
    Point::new(src_port.x, jog_primary),                // vertical to jog
    Point::new(dst_port.x, jog_primary),                // horizontal to dst column
    dst_port,                                           // vertical back up to dst (entering from bottom!)
];
```

Directions left and up follow the same pattern with their respective axes.
...
let (gap_start, gap_end) = match direction {
    ...
    Direction::Left  => (src_anc_rect.right(), dst_anc_rect.x),
    Direction::Down  => (dst_anc_rect.bottom(), src_anc_rect.y),
    Direction::Up    => (src_anc_rect.bottom(), dst_anc_rect.y),
};
```
...
**Simplified implementation (good enough for V1):** Place the label at the midpoint of the enter segment (the last horizontal/vertical segment ending at the destination). This is simpler than the full gap-aware algorithm above and produces acceptable results:

- For cross-container edges, the enter segment spans from the channel (in the gap) to the destination shape. Its midpoint is typically between the gap and the container boundary — usually not overlapping internal nodes.
- For intra-container edges, the enter segment is within the inter-rank gap, so its midpoint is in open space.
- The full gap-aware algorithm (steps 3-4) can be added as a refinement if label overlap proves problematic.

**Offset:** Shift label perpendicular to the segment by `LABEL_OFFSET` (10px). For horizontal segments: shift Y upward (-Y). For vertical segments: shift X leftward (-X). This offset stays within the existing viewBox padding (20px).
...
| **Shape clipping** | Replaced by `compute_port()` (§3.4) for orthogonal routes. `clip_to_shape` retained for Diamond/Hexagon/Circle/Oval shapes. |
| **Direction overrides** | `effective_direction()` (§3.2) reads per-container direction from the LCA. |
| **ViewBox** | `compute_viewbox` already includes all route points. Same-rank jog points (§3.7) extend beyond nodes and are included automatically. No change needed. |
...
- `find_lca` returns the container itself for container-to-descendant edges
...
- `effective_direction` returns graph.direction for root

### Phase 3: Orthogonal Router (Core)

**Files:** `edge_routing.rs`

The router uses three explicit passes:

**Pass 1 — Classify:** For each non-self-loop edge, compute:
- LCA, src_anc, dst_anc
- Effective direction at LCA
- Forward/backward/same-rank classification
- Gap interval (gap_start, gap_end)
- Store classification in a temporary `Vec<EdgeClassification>`

**Pass 2 — Allocate:** With all edges classified:
- Group edges by `(lca, gap_key)` where `gap_key = (gap_start.round(), gap_end.round())`
- Within each gap group: sort by (source cross-position, destination cross-position), assign channel offsets with clamping
- Group edges by source node + exit side: sort by destination cross-position, assign source port positions
- Group edges by destination node + entry side: sort by source cross-position, assign destination port positions

**Pass 3 — Build:** For each edge, construct the route:
- Forward: 4-point three-segment path `[src_port, (channel, src_y), (channel, dst_y), dst_port]`
- Same-rank: 4-point jog path (§3.7)
- Backward: 4-point three-segment with reversed sides (§3.8)
- Degenerate (same cross-position): 2-point straight line
- Container-to-descendant: L-shaped path (§3.2.1)
- Set `route_type = Orthogonal` and `label_position` per §3.9

Update `route_all_edges()` to:
1. Handle self-loops with existing Bézier logic (unchanged)
2. Call the three-pass orthogonal router for all other edges

**Tests:**
- `a -> b` direction=right: straight horizontal line (2 points)
- `a -> b` direction=down: straight vertical line (2 points)
- `a -> b` direction=left: straight horizontal line
- `a -> b` direction=up: straight vertical line
- Cross-rank with Y offset (direction=right): 4-point route (exit, bend, bend, enter)
- Cross-rank with X offset (direction=down): 4-point route
- Same-rank (direction=right): 4-point jog route
- Same-rank (direction=down): 4-point jog route
- Multiple edges same gap (same src, different dsts at same rank): channel positions spread apart, not overlapping
- Multiple edges same gap (different srcs, different dsts): channel positions spread apart
- Cross-container: channel between container boundaries
- Deep nesting (3 levels): correct LCA and channel placement
- Backward edge: reversed port sides
- Bidirectional edges (`a -> b` AND `b -> a`): both route correctly through same gap
- Self-loop: still Bézier, route unchanged
- Container-to-descendant: direct L-shaped route
- Architecture example integration test: all edges have non-empty orthogonal routes, no two channel segments at same position within same gap
...
- SVG for Bézier edge still contains `C` commands (not `L`/`Q`)
- Arrowheads point in correct cardinal direction for all four directions
- Rounded corners don't overshoot (r clamped on short segments)
...
- Label on vertical segment, offset left of the line
- Label not overlapping source or destination nodes
- Multiple edges in same gap have labels at different positions (not piled up)
...
| Direction | Exit side | Enter side | Primary axis | Cross axis | Jog operator |
|-----------|-----------|------------|-------------|------------|-------------|
| Right | right | left | +X | Y | max + JOG |
| Left | left | right | -X | Y | min - JOG |
| Down | bottom | top | +Y | X | max + JOG |
| Up | top | bottom | -Y | X | min - JOG |
...
### 6.7 Same-Side Port Collision (Known V1 Limitation)

When a node has both outgoing forward edges AND incoming same-rank jog edges on the same side (e.g., node B has forward outgoing edges exiting right AND a same-rank jog edge entering from the right), the outgoing and incoming port allocations independently claim the same edge space. Port positions may overlap.

**Impact:** Visual-only — edges still route correctly but may share a port position at the node boundary.

**Frequency:** Uncommon. Requires a node that is simultaneously a same-rank jog destination and a forward edge source, with the jog entering from the same side as the forward exits.

**Future fix:** Combine outgoing and incoming port lists for each side before distributing positions.
...
| Non-rectangular shape ports slightly off-axis | Acceptable small diagonal at shape boundary (only Diamond/Hexagon/Circle/Oval). |
...
| Same-side port collision | Known V1 limitation (§6.7). Uncommon topology. |
